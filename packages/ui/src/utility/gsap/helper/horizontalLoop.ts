/** https://gist.github.com/benjaminrobinet/621cfcb11019897c9ca5cfe9355f8de8 */
...
function horizontalLoop(items: gsap.TweenTarget, config?: HorizontalLoopConfig): HorizontalLoopTimeline {
    ...
    const snap = cfg.snap === false ? (v: number) => v : gsap.utils.snap(cfg.snap as number || 1);
    ...
    function toIndex(index: number, vars?: gsap.TweenVars): gsap.core.Tween {
        ...
        if (Math.abs(idx - curIndex) > length / 2) {
            idx += idx > curIndex ? -length : length;
        }
